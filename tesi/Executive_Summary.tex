...
results in text, and optionally renders a chart. A hierarchical YAML configuration exposes every
inference hyperparameter (sampling parameters, beam width, best-of-$N$, token budgets, and
Chain-of-Thought refinement depth) across open-weight models served locally via Ollama. The
...
open-weight models and then vary one agent step at a time. The results show that model choice
alone does not explain performance: the dominant effects arise from the interaction between model
...
served via Ollama. A hierarchical YAML configuration exposes every inference hyperparameter ---
sampling temperature, top-$p$, top-$k$, beam width, best-of-$N$ candidates, and Chain-of-Thought
...
row-level Intersection-over-Union score for data retrieval, and two LLM-as-judge
...
A hierarchical YAML configuration exposes every inference hyperparameter
\emph{per agent step} --- sampling temperature, top-$p$, top-$k$, beam width, token budget,
...
complementary dimensions: a row-level \emph{Intersection-over-Union} (IoU) between the
...
temperature~1.0, Mistral at~0.15). The agent runs inside a Docker container, with inference
offloaded to the GPU; LLM-as-judge scoring uses the OpenAI API. Experiments were executed on
...
\texttt{repeat\_penalty}~$=1.2$ analysis setting that loses $0.099$ quality and adds
$\sim$90~s/prompt). Two cross-cutting findings stand out: quality-improving settings barely
...
default. A targeted follow-up (Test 04b) confirms the best-of-$N$ selector never preferred a
later candidate: a well-chosen static lookup temperature therefore dominates best-of-$N$ at
lower cost, while CoT remains a qualitatively superior strategy.
...
lookup CoT defines the low-energy region (quality up to 0.921 at 0.0029--0.0045~kWh), while
Gemma~E4B --- repaired through lookup CoT at a lower temperature with leaner downstream token
caps --- defines the high-quality endpoint: 0.937 prompt quality at full completion.
Crucially, against the untuned baselines this is not a quality-for-energy trade but a genuine
Pareto improvement: the best Gemma~E4B candidate raises prompt quality by $+0.102$ \emph{and}
cuts energy (0.00602 vs.\ 0.00883~kWh), and the best Mistral candidate likewise gains $+0.077$
quality at lower energy (0.00449 vs.\ 0.00602~kWh), so both recommended configurations
dominate their own Test~01 baselines on both axes. The Gemma~E4B endpoint still costs about
34\,\% more energy than the cheapest Mistral candidate (0.00602 vs.\ 0.00449~kWh), and
Gemma~26B improves with tuning but stays dominated once energy counts. Splitting by difficulty
explains the trade-off, and reliability --- not just score --- is what separates the two
leaders. On easy-to-medium prompts Mistral is the best economic choice (quality $\approx
0.98$). On the hardest difficulty-4 prompts, however, Mistral plateaus at 0.831 quality and
\emph{fails to complete} about 9\,\% of prompts (90.9\,\% completion), whereas the repaired
Gemma~E4B reaches 0.919 quality at full completion. Gemma~E4B therefore becomes preferable
whenever the workload includes hard SQL aggregations and robust completion matters more than
minimum energy: the best model for simple tasks is not the best for hard ones.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{Images/test05_final_run_unified_candidate_pareto.png}
    \caption{Test~05: unified quality--energy Pareto frontier over the final candidate set.}
    \label{fig:es_pareto}
\end{figure}
...
overtake it. This repair comes specifically from CoT refinement, which exploits executable
SQL feedback; best-of-$N$ sampling, by contrast, gave no benefit beyond its temperature
setting, since the selector never preferred the second candidate. After tuning, the
quality--energy frontier is shared between low-energy Mistral
candidates and a single high-quality Gemma~E4B endpoint, while the larger Gemma~26B
improves yet stays dominated once energy is counted. Notably, the recommended configurations
dominate their own untuned baselines on both axes --- higher quality at lower energy --- so
per-step tuning is a genuine improvement rather than a quality-for-energy trade.
