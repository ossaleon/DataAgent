...
Union (IoU) metric. Before comparison, both DataFrames are passed through a
normalisation step (\texttt{normalize\_dataframe\_values}) that: casts
float-encoded integers to their integer representation (e.g.\ \texttt{33653.0}
$\to$ \texttt{33653}), rounds floating-point values to two decimal places,
formats date columns as \texttt{YYYY-MM-DD} strings, and strips leading and
trailing whitespace from string cells. Column names are additionally lowercased
on both sides to make the comparison case-insensitive.
...
\texttt{YYYY-MM-01} as equivalent. The score is defined as:
...
columns are compared; if no columns are shared the score is 0.

\paragraph{Text Score (\texttt{text\_score}).}
The quality of the natural language analysis is evaluated using an LLM-as-judge
approach~\cite{zheng2023judging}. A separate LLM call (temperature 0, \texttt{gpt-4o-mini}
by default) receives the generated analysis alongside the ground-truth reference text
...
The quality of the generated chart is evaluated by a second LLM judge that receives
