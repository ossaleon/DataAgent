...
This section describes the Sales Agent implemented for the thesis.
Section~\ref{subsec:overview} presents the system overview, while
Sections~\ref{subsec:architecture} and~\ref{subsec:langgraph} detail the system
architecture and the LangGraph workflow that orchestrates its steps.
Section~\ref{subsec:tools} describes the individual agent tools and steps used
for data lookup, textual analysis, and visualization, and
Section~\ref{subsec:prompting} explains the LLM integration and prompting
techniques. Section~\ref{subsec:config} presents the YAML-based configuration
system, and Section~\ref{subsec:multitable} describes the multi-table dataset
support. Finally, Section~\ref{subsec:benchmark} presents the benchmark design
and Section~\ref{subsec:evaluation} details the evaluation, output, and
energy-monitoring pipeline used to profile the agent.
...
all per-step hyperparameters used by the experiments: number of best-of-$n$
candidates, temperature range, top-$p$, top-$k$, maximum tokens, caching policy, and optional evaluation
...
OpenTelemetry~\cite{opentelemetry}, with compatibility for the Arize Phoenix
platform~\cite{arize_phoenix}. The integration is implemented in the codebase:
when enabled, it records structured spans for each processing step, capturing
inputs, outputs, latencies, and evaluation scores for post-hoc analysis.
However, Phoenix was not used during the experiments reported in this thesis;
the experimental analysis relies on the benchmark CSV outputs and CodeCarbon
measurements.
...
\begin{itemize}
    ...
    \item \textbf{Matplotlib} — chart rendering for the visualization step;
    \item \textbf{CodeCarbon} — energy consumption tracking during agent execution;
    \item \textbf{OpenTelemetry} / \textbf{Arize Phoenix} — optional tracing and
          observability integration; implemented in the framework but not used
...
(1)~\texttt{lookup\_sales\_data} must run before any analysis or visualization;
(2)~no tool may be selected more than once per execution;
(3)~\texttt{end} is selected when both the textual analysis and the visualization answers
have been generated, or when all tools relevant to the user's request have been executed.
...
Sales Agent on a question that requires data retrieval, analysis, and visualization.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{Images/fig4_sequence.png}
    \vspace{-6pt}
...
analysis and visualization, returning only the raw query result; \texttt{analysis}
skips visualization and returns data plus text analysis. These modes reduce latency
...
together with the user prompt and a structured SQL-generation prompt containing
DuckDB-specific instructions, common mistakes to avoid, and few-shot examples.
It then produces a SQL query. The query is executed via
\texttt{duckdb.execute()}, and the result is stored in the \texttt{State} as both
a CSV string (\texttt{data}) and a Pandas \texttt{DataFrame}
(\texttt{data\_df}).
...
role declaration that assigns a persona to the model (e.g.\
\textit{``You are an expert SQL developer specialising in DuckDB queries''}), followed
by a Markdown-sectioned body that separates concerns into labeled blocks:
...
and asking it to either reproduce it exactly when it is correct and complete — leaving
the parts that are already correct unchanged and revising only the errors — or produce
an improved version otherwise. The loop runs for at most \texttt{cot\_n} total iterations,
but stops early as soon as the output converges — i.e.\ when the textual similarity
between two consecutive responses reaches or exceeds 0.95, measured as the character-level
\texttt{difflib.SequenceMatcher} ratio (defined as $2M/T$, where $T$ is the total number
of characters in the two responses and $M$ is the number matched by the longest contiguous
matching subsequences) — avoiding redundant refinement calls once the model has settled on
a stable answer.

Two refinement modes are available. In the standard mode, the model receives its
previous output and can voluntarily refine it. In error-aware mode, triggered whenever
a step reports an execution error — either a SQL query that fails to execute in the
lookup step or visualization code that raises an exception during validation — the error
message is explicitly included in the refinement block with a directive to fix it. This
...
aspect of the agent's behaviour without requiring code changes. All behavioural settings
are defined in a single YAML file, loaded at startup via \texttt{AgentConfig.from\_yaml()},
which additionally discovers the per-table \texttt{*\_schema.yaml} files referenced by the
\texttt{schema} section.
...
\begin{itemize}
    ...
    \item \texttt{run} — execution parameters: prompt, visualization goal, operating
    ...
    \item \texttt{tracing} — optional OpenTelemetry/Phoenix endpoint for observability.
\end{itemize}
...
single execution without modifying the base configuration: before the run starts,
the agent deep-copies its configuration and applies the overrides on the copy. For
example, setting \texttt{analyzing\_data.n: 3} together with \texttt{temp\_max: 1.3}
enables best-of-$3$ sampling for the analysis step --- widening its temperature
range --- in that run only.

A separate, complementary mechanism is the pluggable \texttt{ParameterProvider}
abstraction, queried before each step to return the effective \texttt{StepConfig}.
The default \texttt{DefaultProvider} returns the configuration unchanged and is the
provider in effect for all experiments reported in this thesis, which run
non-interactively. The codebase also ships an optional \texttt{TerminalProvider}
which, when the agent is launched from an interactive terminal (i.e.\ when standard
input is a TTY), displays each step's parameters and lets the user accept or override
them one by one before that step executes, falling back to the defaults otherwise.
This interactive facility is intended for manual, exploratory use and is not exercised
by the benchmark runs reported here.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{gt\_chart\_code}} — reference Python/Matplotlib code that
          renders the correct visualization from \texttt{gt\_data};
    \item \textbf{\texttt{explicit\_requirements}} — a structured dictionary of
          ...
          grid, markers) used to tighten the visualization evaluation;
...
to~4. Prompt length, the presence of a visualization request, and the number of
...
\paragraph{Ground-truth table similarity (\texttt{csv\_iou}).}
The quality of the data retrieval step is measured by comparing the agent's
result \texttt{DataFrame} with the ground-truth CSV. The output column is still
named \texttt{csv\_iou} for continuity with earlier experiments, but the final
benchmark scorer is a graded ground-truth table-similarity metric rather than a
brittle exact row-IoU score. This change is important because many correct SQL
answers differ from the reference only in harmless surface choices: column
aliases, column order, boolean labels, numeric formatting, or long-versus-wide
layout.

Failed SQL executions, missing dataframes, and empty results receive score~0.
Otherwise, both tables are first normalised with
\texttt{normalize\_dataframe\_values}: float-encoded integers are cast to their
integer representation, numeric values are rounded, dates are rendered as
\texttt{YYYY-MM-DD}, and string cells are stripped of leading and trailing
whitespace. Column names are lowercased before benchmark evaluation; during
agent execution, result columns can also be aligned to the ground-truth column
order when ground-truth columns are available.

The scorer then computes the best of two comparisons. The direct comparison
aligns columns by a weighted combination of normalised column-name similarity
and value similarity, so aliases such as \texttt{revenue} and
\texttt{total\_\allowbreak sales\_\allowbreak value}, or swapped numeric columns with matching values,
can still receive high credit. Rows are matched without assuming order, using a
soft value similarity that gives full credit for equal values, month/month-start
date equivalence, near-equal numeric values within tolerance, and common binary
category equivalents such as \texttt{true}/\texttt{false},
\texttt{1}/\texttt{0}, \texttt{Organic}/\texttt{Non-Organic}, and
\texttt{Promo}/\texttt{Non-Promo}. The final direct score combines table
structure and soft row-value agreement. A second comparison detects common
long-versus-wide binary grouped outputs, such as
\texttt{month, promo\_flag, revenue} versus
\texttt{month, promo\_revenue, non\_promo\_revenue}, and the final
\texttt{csv\_iou} value is the maximum of the direct and long-wide scores.
...
\noindent The two criteria are combined with equal weight because Factual
Accuracy and Coverage capture two independently necessary and
non-substitutable properties of a good analysis---correctness of the reported
values and completeness with respect to the reference---so neither is
privileged a priori. The averaged rubric score is then rescaled from the
judge's native $[1,5]$ range to $[0,1]$ through an affine transformation
that preserves the ordering and relative spacing of the scores while placing
\texttt{text\_score} on the same $[0,1]$ scale as \texttt{csv\_iou} and
\texttt{vis\_score}. This makes the three step metrics directly comparable and
lets them be averaged into a single \texttt{quality\_mean} (the arithmetic mean
of the three component score means, Equation~\ref{eq:quality_mean}) without any
dimension dominating purely because of a wider numeric range. Using a strong LLM as a judge to rate
generated outputs on a discrete scale is an established evaluation approach
whose scores have been shown to align well with human
judgement~\cite{zheng2023judging}.

\paragraph{Visualization Score (\texttt{vis\_score}).}
The quality of the generated chart is evaluated by a second LLM judge (\texttt{gpt-5.4}, temperature 0.2) that receives
...
\noindent The weights always sum to one, keeping \texttt{vis\_score} on a fixed
$[0,1]$ scale, and rank the criteria by how directly they affect whether the
chart conveys the intended information. Axis Correctness and Chart Type
Correctness receive an equal $0.25$ each as the two fundamental structural
decisions of a plot---\emph{what} is plotted and \emph{how}. Functional
Equivalence carries the largest single weight because it is the most
comprehensive criterion, capturing whether the fully rendered chart is visually
equivalent to the reference rather than any isolated attribute. Explicit
Requirements Compliance is deliberately capped at $0.10$: styling constraints
such as colour scheme or title format are cosmetic refinements, so this cap
ensures that an otherwise correct chart cannot be heavily penalised for a purely
stylistic deviation. When the benchmark item specifies no explicit requirements
the criterion is not applicable, and rather than renormalising all four
weights---which would also inflate the structural criteria---its $0.10$ share is
reassigned to Functional Equivalence, the most encompassing of the remaining
criteria. This keeps the relative emphasis on substantive correctness stable and
the resulting \texttt{vis\_score} directly comparable across items with and
without explicit styling constraints. Relying on an LLM judge to rate the
generated chart against the reference follows the same established
LLM-as-judge practice~\cite{zheng2023judging}, whereas the specific criteria
and their weights are a design choice tuned to the priorities of this
benchmark.
...
For the analysis and visualization steps, the no-GT evaluator uses an LLM judge
...
As already mentioned, energy consumption is tracked using
CodeCarbon~\cite{lottick2019codecarbon}, an
...
output CSV file. Each row corresponds to one benchmark item and
...
\hline
\texttt{csv\_iou\_reasoning}, \texttt{text\_score\_reasoning}, \texttt{vis\_score\_reasoning} & Diagnostic message explaining the mismatch populated only when the corresponding GT score $< 1.0$ \T\B \\
\hline
...
\hline
\texttt{\{step\}\_llm\_energy\_kwh}, \texttt{\{step\}\_llm\_{cpu|gpu|ram}\_} \texttt{energy\_kwh}, \texttt{\{step\}\_llm\_emissions\_co2} & Per-step energy of the LLM calls --- total, CPU, GPU and RAM energy (kWh) and CO$_2$ emissions (kg) --- for each of the 3 steps (5 fields $\times$ 3 steps, as described in the Energy Consumption Monitoring subsection above); \texttt{None} when CodeCarbon is disabled \T\B \\
\hline
...
\hline
\end{tabular}
\caption{Main columns of the output CSV.}
