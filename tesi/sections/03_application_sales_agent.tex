...
This section describes the Sales Agent implemented for the thesis. It first
presents the system overview, architecture, and LangGraph workflow, then details
the individual agent tools and steps used for data lookup, textual analysis, and
visualization. The section then explains the LLM integration and prompting
techniques, the YAML-based configuration system, and the multi-table dataset
support. It closes by describing the benchmark design and the evaluation,
output, and energy-monitoring pipeline used to profile the agent.
...
\texttt{total\_sales\_value}, or swapped numeric columns with matching values,
...
$[1, 5]$ to $[0, 1]$  \textbf{XXX dare un razionale per questo, se avete trovato qualcosa di simile in letteratura indicatelo  XXX}:
...
and $w_f = 0.50$, $w_e = 0$ otherwise  \textbf{XXX anche qui dare il razionale XXX}.
