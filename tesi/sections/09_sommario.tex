...
LangGraph, che traduce una domanda di business in una query SQL eseguita su file Parquet tramite
DuckDB, ne interpreta i risultati in forma testuale e, opzionalmente, genera un grafico. Una
configurazione gerarchica in YAML espone tutti gli iperparametri di inferenza (parametri di
campionamento, ampiezza del beam, best-of-$N$, budget di token e profondità del raffinamento
Chain-of-Thought) sia per modelli open-weight serviti localmente tramite Ollama, sia per modelli
cloud tramite l'API OpenAI (qui usata unicamente per la valutazione LLM-as-judge). Il contributo
...
dimensioni: un punteggio Intersection-over-Union a livello di riga per il recupero dei dati e due
...
Mistral~Small~3.2~24B) e poi variano uno step dell'agente alla volta su parametri di
campionamento, budget di token e compute a tempo di inferenza, concludendo con una conferma di
Pareto. I risultati mostrano che la sola scelta del modello non spiega le prestazioni: gli
