...
# Run final thesis Tests 03 and 04 sequentially through Docker.
...
usage() {
  cat <<'EOF'
Usage:
  bash evaluation/run_thesis_final_tests_03_04_docker.sh [--dry-run]

Runs final thesis Tests 03 and 04 sequentially with Docker and --resume.
...
cd "$ROOT_DIR"
printf 'Running final thesis Tests 03 and 04 sequentially.\n'
printf 'Docker image: %s | GPU device: %s | runs mount: %s\n' "$IMAGE_NAME" "$GPU_DEVICE" "$RUNS_HOST_DIR"
run_test03
run_test04
printf '\nFinal thesis Tests 03 and 04 finished.\n'
