# Test 01: Baseline Model Comparison
...
No agent-specific parameter is varied in this test. The same model acts as both generator and judge:

```text
--model <tested-model>
--judge-model <tested-model>
```

Execution manifests:

```text
gemma4:e4b             evaluation/thesis_test01_gemma4_baseline.yaml
gemma4:26b             evaluation/thesis_test01_gemma4_baseline.yaml
mistral-small3.2:24b   evaluation/thesis_test01_mistral_small32_baseline.yaml
```
...
for REP in 01 02 03; do
  docker run --rm --gpus '"device=0"' --network=host \
    ...
    evaluation/thesis_test01_gemma4_baseline.yaml \
    --provider ollama \
    --model gemma4:e4b \
    --judge-provider ollama \
    --judge-model gemma4:e4b \
    --save-dir runs/thesis_tests/01_model_baseline_comparison/gemma4_e4b/rep${REP} \
    --resume
  ...
  docker run --rm --gpus '"device=0"' --network=host \
    ...
    --save-dir runs/thesis_tests/01_model_baseline_comparison/gemma4_26b/rep${REP} \
    --resume
  ...
  docker run --rm --gpus '"device=0"' --network=host \
    ...
    evaluation/run_manifest_benchmark.py \
    evaluation/benchmark_dataset.json \
    evaluation/thesis_test01_mistral_small32_baseline.yaml \
    --provider ollama \
    --model mistral-small3.2:24b \
    --judge-provider ollama \
    --judge-model mistral-small3.2:24b \
    --save-dir runs/thesis_tests/01_model_baseline_comparison/mistral_small32_24b/rep${REP} \
    --resume
done
```
